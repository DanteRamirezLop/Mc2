...
class Ambiente extends DB{
    ...
    function obtenerAmbiente($Id_foraneo){
        $query = $this->connect()->query('SELECT a. id ,a. idProyecto, a. nAmbiente, a. largo, a. ancho, a. altura, a. coordenadas FROM ambiente a INNER JOIN proyecto p ON a.idProyecto = p.id WHERE p.id ='.$Id_foraneo);  
        return $query;
    }
}
