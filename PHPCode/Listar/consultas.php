...
class Ducto extends DB{
    ...
    function obtenerDucto(){
		...
		$query = $this->connect()->query('SELECT * FROM item JOIN ducto ON item.idItem = ducto.id INNER JOIN ductoex ON ducto.id = ductoex.idDucto');  
		return $query;
    }
}
...
class Rejilla extends DB{
    ...
    function obtenerRejilla(){
		...
		$query = $this->connect()->query('SELECT * FROM item INNER JOIN rejilla ON item.idItem = rejilla.id');
        return $query;
    }
}
...
class Multiple extends DB{
    ...
    function obtenerMultiple(){
		...
		$query = $this->connect()->query('SELECT * FROM item INNER JOIN multiple ON item.idItem = multiple.id');
        return $query;
    }
}
